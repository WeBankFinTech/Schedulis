...
public class FlowUtils {
    ...
    public static void applyDisabledJobs(final List<Object> disabledJobs,
        ...
        for (final Object disabled : disabledJobs) {
            if (disabled instanceof String) {
                ...
                if (node != null) {
                    List<Object> autoDisabledJobs = exflow.getExecutionOptions()
                        .getAutoDisabledJobs();
                    // 存在 auto.disabled 属性，且当前为 false 的节点不作处理
                    if (!(autoDisabledJobs.contains(node.getId()) && !node.isAutoDisabled())) {
     ...
     * @param disabledJobs
     * @param exflow
     * @param autoDisabledJobs
     */
    public static void applyAutoDisabledJobs(List<Object> disabledJobs, ExecutableFlowBase exflow,
        List<Object> autoDisabledJobs) {
        for (final Object disabled : disabledJobs) {
            if (disabled instanceof String) {
                final String nodeName = (String) disabled;
                final ExecutableNode node = exflow.getExecutableNode(nodeName);
                if (node != null) {
                    if (node.isAutoDisabled()) {
                        autoDisabledJobs.add(node.getId());
                    }
                } else {
                    for (ExecutableNode executableNode : exflow.getExecutableNodes()) {
                        if (executableNode instanceof ExecutableFlowBase) {
                            ArrayList<Object> disabledJob = new ArrayList<>();
                            disabledJob.add(disabled);
                            applyAutoDisabledJobs(disabledJob, (ExecutableFlowBase) executableNode,
                                autoDisabledJobs);
                        }
                    }
                }
                ...
                if (node instanceof ExecutableFlowBase) {
                    applyAutoDisabledJobs(subDisabledJobs, (ExecutableFlowBase) node,
                        autoDisabledJobs);
                }
            }
        }
        ...
        for (final Object enabled : enabledJobs) {
            if (enabled instanceof String) {
                ...
                if (node != null) {
                    ...
                    if (isFlowNode){
                        setAllFlowEnabled((ExecutableFlowBase) node);
                    }
    ...
    public static void setExflow(ExecutableFlowBase exflow, List<Object> enabledJobs) {

        for (int i = 0; i < enabledJobs.size(); i++) {
            Object nodeName = enabledJobs.get(i);
            ExecutableNode executableNode = exflow.getExecutableNode(nodeName.toString());
            if (executableNode != null) {
                executableNode.setStatus(Status.READY);
            }
            if (executableNode != null && i== enabledJobs.size() -1 ) {
                executableNode.setStatus(Status.READY);
                boolean isFlowNode = executableNode.getType().equals(SpecialJobTypes.EMBEDDED_FLOW_TYPE);
                if (isFlowNode){
                    ExecutableFlowBase childExecuFlow = (ExecutableFlowBase) executableNode;
                    setAllFlowEnabled(childExecuFlow);
                }
    ...
    public static void setAllFlowDisabled(ExecutableFlowBase exflow) {
        if (exflow != null) {
            for (ExecutableNode exeNode : exflow.getExecutableNodes()) {
                exeNode.setStatus(Status.DISABLED);
                if (exeNode instanceof ExecutableFlowBase){
                    setAllFlowDisabled((ExecutableFlowBase) exeNode);
                }
    ...
    public static void setAllFlowEnabled(ExecutableFlowBase exflow) {
        if (exflow != null) {
            for (ExecutableNode exeNode : exflow.getExecutableNodes()) {
                exeNode.setStatus(Status.READY);
                boolean isFlowNode = exeNode.getType().equals(SpecialJobTypes.EMBEDDED_FLOW_TYPE);
                if (isFlowNode){
                    setAllFlowEnabled((ExecutableFlowBase) exeNode);
                }
