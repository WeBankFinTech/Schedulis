 ...
 * @date 2022/03/24
 **/
public class DataMaskingParameterizedMessage implements Message, StringBuilderFormattable {
  ...
  // storing JDK classes in ThreadLocals does not cause memory leaks in web apps,
  // so this is okay
   ...
   */
  @Deprecated
  public DataMaskingParameterizedMessage(final String messagePattern, final String[] arguments,
   ...
   * @param throwable      A Throwable.
   */
  public DataMaskingParameterizedMessage(final String messagePattern, final Object[] arguments,
      final Throwable throwable) {
    argArray = arguments;
    this.throwable = throwable;
    init(messagePattern);
   ...
   */
  public DataMaskingParameterizedMessage(final String messagePattern, final Object arg) {
   ...
   */
  public DataMaskingParameterizedMessage(final String messagePattern, final Object arg0,
      final Object arg1) {
  ...
  private void init(final String messagePattern) {
    ...
    if (messagePattern != null && messagePattern.length() < 10) {
      this.messagePattern = DataMaskUtil.doDataMask(messagePattern, true);
    } else {
      this.messagePattern = DataMaskUtil.doDataMask(messagePattern);
    }
    final int len = Math
        .max(1, this.messagePattern == null ? 0 : this.messagePattern.length() >> 1); // divide by 2
    indices = new int[len]; // LOG4J2-1542 ensure non-zero array length
    final int placeholders = ParameterFormatter
        .countArgumentPlaceholders2(this.messagePattern, indices);
    initThrowable(argArray, placeholders);
    usedCount = Math.min(placeholders, argArray == null ? 0 : argArray.length);
  }
  ...
  public boolean equals(final Object o) {
    ...
    final DataMaskingParameterizedMessage that = (DataMaskingParameterizedMessage) o;

    if ((messagePattern != null ? !messagePattern.equals(that.messagePattern)
        : that.messagePattern != null) || !Arrays.equals(argArray, that.argArray)) {
      return false;
    }
    // if (throwable != null ? !throwable.equals(that.throwable) : that.throwable !=
    // null) return false;
  ...
  public int hashCode() {
    ...
    return HASHVAL * result + (argArray != null ? Arrays.hashCode(argArray) : 0);
  }
   ...
   * </p>
   * <blockquote> As much as is reasonably practical, the hashCode method defined
   * by class {@code Object} does return distinct integers for distinct objects. (This is typically
  ...
  public String toString() {
    return "ParameterizedMessage[messagePattern=" + messagePattern + ", stringArgs=" + Arrays
        .toString(argArray) + ", throwable=" + throwable
        + ']';
  }
}
