...
import cn.webank.weup.base.enums.EnumSysPropKeyAble;
import cn.webank.weup.base.enums.WeupBaseDataMaskKey;
import cn.webank.weup.base.log.Slf4jUtil;
import cn.webank.weup.base.util.DataMaskUtil;
...
final class ParameterFormatter {
  ...
  static int countArgumentPlaceholders(final String messagePattern) {
    ...
    for (int i = 0; i < length - 1; i++) {
      ...
      if (curChar == ESCAPE_CHAR) {
        ...
        if ((curChar == DELIM_START) && (!isEscaped
   ...
   * @return the number of unescaped placeholders.
   */
  static int countArgumentPlaceholders3(final char[] messagePattern, final int length,
      final int[] indices) {
    int result = 0;
    boolean isEscaped = false;
    for (int i = 0; i < length - 1; i++) {
      ...
      if (curChar == ESCAPE_CHAR) {
        ...
        if ((curChar == DELIM_START) && (!isEscaped && messagePattern[i + 1] == DELIM_STOP)) {
          indices[result] = i;
          result++;
          i++;
        }
   ...
   * @param arguments      the arguments to be used to replace placeholders.
   */
  static void formatMessage3(final StringBuilder buffer, final char[] messagePattern,
      final int patternLength, final Object[] arguments, final int argCount,
      final int[] indices) {
    if (messagePattern == null) {
      return;
    }
  ...
  /**
   * Returns {@code true} if the specified char and the char at {@code curCharIndex + 1} in the
   * specified message pattern together form a "{}" delimiter pair, returns {@code false}
   * otherwise.
   */
  ...
  private static boolean isDelimPair(final char curChar, final String messagePattern,
      final int curCharIndex) {
    return curChar == DELIM_START && messagePattern.charAt(curCharIndex + 1) == DELIM_STOP;
  }

  /**
   * Detects whether the message pattern has been fully processed or if an unprocessed character
   * remains and processes it if necessary, returning the resulting position in the result char
   * array.
   */
  // Profiling showed this method is important to log4j performance. Modify with
  // care!
  ...
  /**
   * Processes the last unprocessed character and returns the resulting position in the result char
   * array.
   */
  // Profiling showed this method is important to log4j performance. Modify with
  // care!
  // 28 bytes (allows immediate JVM inlining: < 35 bytes) LOG4J2-1096
  private static void handleLastChar(final StringBuilder buffer, final int escapeCounter,
      final char curChar) {
    if (curChar == ESCAPE_CHAR) {
  ...
  /**
   * Processes a literal char (neither an '\' escape char nor a "{}" delimiter pair) and returns the
   * resulting position.
   */
  // Profiling showed this method is important to log4j performance. Modify with
  // care!
  // 16 bytes (allows immediate JVM inlining: < 35 bytes) LOG4J2-1096
  private static void handleLiteralChar(final StringBuilder buffer, final int escapeCounter,
      final char curChar) {
  ...
  /**
   * Writes "{}" to the specified result array at the specified position and returns the resulting
   * position.
   */
  // Profiling showed this method is important to log4j performance. Modify with
  // care!
  ...
  /**
   * Writes a '\' char to the specified result array (starting at the specified position) for each
   * <em>pair</em> of '\' escape chars encountered in the message format and returns the resulting
   * position.
   */
  // Profiling showed this method is important to log4j performance. Modify with
  // care!
  ...
  /**
   * Writes the specified number of '\' chars to the specified result array (starting at the
   * specified position) and returns the resulting position.
   */
  // Profiling showed this method is important to log4j performance. Modify with
  // care!
  ...
  /**
   * Appends the argument at the specified argument index (or, if no such argument exists, the "{}"
   * delimiter pair) to the specified result char array at the specified position and returns the
   * resulting position.
   */
  // Profiling showed this method is important to log4j performance. Modify with
  // care!
  // 25 bytes (allows immediate JVM inlining: < 35 bytes) LOG4J2-1096
  private static void writeArgOrDelimPair(final Object[] arguments, final int argCount,
      final int currentArgument, final StringBuilder buffer) {
  ...
  private static boolean appendSpecialTypes(Object o, final StringBuilder str) {
    if (o instanceof CharSequence) {
      String strTmp = o.toString();
      o = Slf4jUtil.doDataMask(strTmp);
    }
    if (o instanceof Float || o instanceof Double) {
      double dTmp = ((Number) o).doubleValue();
      if (EnumSysPropKeyAble
          .isTrueForSysTemProperty(WeupBaseDataMaskKey.WEUP_DATMASK_NEED_MASK_FLOAT)) {
        if (dTmp >= 0) {
          o = DataMaskUtil.NUMBER_DATA_MASKING_STRING + dTmp % 1000.0;
        } else {
          o = "-" + DataMaskUtil.NUMBER_DATA_MASKING_STRING + -dTmp % 1000.0;
        }
      }
    }
    if ((o instanceof BigDecimal) && EnumSysPropKeyAble
        .isTrueForSysTemProperty(WeupBaseDataMaskKey.WEUP_DATMASK_NEED_MASK_FLOAT)) {
      String strTmp = ((BigDecimal) o).toPlainString();
      int index = strTmp.indexOf(".");
      if (index > 3) {// 需要脱敏的, 整数保留3位，然后统一6位星号
        String dmStr = strTmp.substring(index - 3);
        if (strTmp.startsWith("-")) {
          o = "-" + DataMaskUtil.NUMBER_DATA_MASKING_STRING + dmStr;
        } else {
          o = DataMaskUtil.NUMBER_DATA_MASKING_STRING + dmStr;
        }
      }
  ...
  private static void tryObjectToString(final Object o, final StringBuilder str) {
    ...
    try {
      String strTmp = Slf4jUtil.doDataMask(o.toString());
      str.append(strTmp);
   ...
   * </p>
   * <blockquote> As much as is reasonably practical, the hashCode method defined by class {@code
   * Object} does return distinct integers for distinct objects. (This is typically implemented by
