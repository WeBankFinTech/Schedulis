...
public class ExecutionController extends EventHandler implements ExecutorManagerAdapter {
    ...
    public List<ExecutableFlow> getMaintainedFlowsQuickSearch(String flowIdContains,
                                                              int skip, int size, String username, List<Integer> projectIds)
            throws ExecutorManagerException {
    ...
    public List<ExecutableJobInfo> getExecutableJobs(final Project project,
                                                     final String jobId, final int skip, final int size,String nestedId) throws ExecutorManagerException {
        final List<ExecutableJobInfo> nodes =
                this.executorLoader.fetchJobHistory(project.getId(), jobId, skip, size,nestedId);
        return nodes;
    }
    ...
    @Override
    public int getNumberOfJobExecutions(final Project project, final String jobId,String nestedId)
            throws ExecutorManagerException {
        return this.executorLoader.fetchNumExecutableNodes(project.getId(), jobId,nestedId);
    }
    ...
    public Long getLatestLogOffset(ExecutableFlow exFlow, String jobId, Long length, int attempt,
                                   User user) throws ExecutorManagerException {
        return 0L;
    }
    ...
    @Override
    public void setFlowFailed(ExecutableFlow exFlow, String userId, List<Pair<String, String>> param)
            throws Exception {
        synchronized (exFlow) {
            final Pair<ExecutionReference, ExecutableFlow> pair =
                    this.executorLoader.fetchActiveFlowByExecId(exFlow.getExecutionId());
    ...
    public String retryFailedJobs(ExecutableFlow exFlow, String userId, String request)
        ...
        synchronized (exFlow) {
            ...
            String url = "http://" + pair.getFirst().getExecutor().get().getHost() + ":" + pair.getFirst()
                    .getExecutor().get().getPort() + "/executor?"
    ...
    public String submitExecutableFlow(final ExecutableFlow exflow, final String userId)
        ...
        synchronized (exFlowKey.intern()) {
            ...
            if (options.getDisabledJobs() != null && !options.getDisabledJobs().isEmpty()) {
                ...
                FlowUtils.setAllFlowDisabled(exflow);
                if (enabledJobs.contains("noChild")) {
                    enabledJobs.forEach(node -> {
                        String[] split = node.toString().split(":");
                        List<Object> jobs = new ArrayList<>();
                        jobs.addAll(Arrays.asList(split));
                        //最后一个节点若还有节点未填写，将最后节点的子节点全部设置为ready
                       // setLastFlow(exflow,jobs);
                        FlowUtils.setExflow(exflow, jobs);
                    });
                    ...
                    for (int i = 0; i < enabledJobs.size(); i++) {
                        ...
                        if (enabled instanceof String) {
                            ...
                            //当前flow存在子job时，子job全部设置成READY
                            boolean isFlowNode = node.getType().equals(SpecialJobTypes.EMBEDDED_FLOW_TYPE);
                            if (isFlowNode){
                                ExecutableFlowBase thisExecFlow = (ExecutableFlowBase) node;
                                FlowUtils.setAllFlowEnabled(thisExecFlow);
                            }
                            if (node != null) {
    ...
    public static void setLastFlow(ExecutableFlowBase exflow, List<Object> jobs) {

        for (int i = 0; i < jobs.size(); i++) {
            ExecutableNode executableNode = exflow.getExecutableNode(jobs.get(i).toString());
            if (executableNode instanceof ExecutableFlowBase){
                exflow = (ExecutableFlowBase) executableNode;
                if (i==jobs.size()-1){
                    exflow.getExecutableNodes().forEach(node->{
                        node.setStatus(Status.READY);
                    });
                }
            }
    ...
    @Override
    public List<ExecutableFlow> getExecutableFlows(Project project, String flowId, int skip, int size)
            throws ExecutorManagerException {
        return null;
    }

    @Override
    public long getExecutableJobsMoyenneRunTime(Project project, String jobId)
            throws ExecutorManagerException {
        return 0;
    }
    ...
    @Override
    public FileIOUtils.JobMetaData getExecutionJobMetaData(ExecutableFlow exFlow, String jobId,
                                                           int offset, int length, int attempt) throws ExecutorManagerException {
        return null;
    }

    @Override
    public void pauseExecutingJobs(ExecutableFlow exFlow, String userId, String... jobIds)
            throws ExecutorManagerException {

    }

    @Override
    public void resumeExecutingJobs(ExecutableFlow exFlow, String userId, String... jobIds)
            throws ExecutorManagerException {
        // implements
    }
    ...
    @Override
    public void disableExecutingJobs(ExecutableFlow exFlow, String userId, String... jobIds)
            throws ExecutorManagerException {
        // implements
    }

    @Override
    public void enableExecutingJobs(ExecutableFlow exFlow, String userId, String... jobIds)
            throws ExecutorManagerException {
        // implements
    }

    @Override
    public void cancelExecutingJobs(ExecutableFlow exFlow, String userId, String... jobIds)
            throws ExecutorManagerException {
        // implements
    }
    ...
    @Override
    public List<ExecutableFlow> getHistoryRecoverExecutableFlows(String userNameContains)
            throws ExecutorManagerException {
        return null;
    }

    @Override
    public ExecutableFlow getHistoryRecoverExecutableFlowsByRepeatId(String repeatId)
            throws ExecutorManagerException {
        return null;
    }

    @Override
    public void stopHistoryRecoverExecutableFlowByRepeatId(String repeatId)
            throws ExecutorManagerException {
        // implements
    }

    @Override
    public ExecutableFlow getHistoryRecoverExecutableFlowsByFlowId(String flowId, String projectId)
            throws ExecutorManagerException {
        return null;
    }

    @Override
    public List<ExecutionRecover> listHistoryRecoverFlows(Map paramMap, int skip, int size)
            throws ExecutorManagerException {
        return null;
    }

    @Override
    public List<ExecutionRecover> listMaintainedHistoryRecoverFlows(String username,
                                                                    List<Integer> projectIds, int skip, int size)
            throws ExecutorManagerException {
        return null;
    }
    ...
    @Override
    public Integer saveHistoryRecoverFlow(ExecutionRecover executionRecover)
            throws ExecutorManagerException {
        return null;
    }

    @Override
    public void updateHistoryRecover(ExecutionRecover executionRecover)
            throws ExecutorManagerException {
        // implements
    }
    ...
    @Override
    public ExecutionRecover getHistoryRecoverFlowByPidAndFid(String projectId, String flowId)
            throws ExecutorManagerException {
        return null;
    }

    @Override
    public List<ExecutionRecover> listHistoryRecoverRunnning(Integer loadSize)
            throws ExecutorManagerException {
        return null;
    }
    ...
    @Override
    public int getMaintainedHistoryRecoverTotal(String username, List<Integer> maintainedProjectIds)
            throws ExecutorManagerException {
        return 0;
    }

    @Override
    public String getDownLoadAllExecutionLog(ExecutableFlow executableFlow)
            throws ExecutorManagerException {
        return null;
    }
    ...
    @Override
    public int getExecHistoryQuickSerachTotal(Map<String, String> filterMap)
            throws ExecutorManagerException {
        return 0;
    }

    @Override
    public int getMaintainedFlowsQuickSearchTotal(String username,
                                                  final Map<String, String> filterMap, List<Integer> projectIds)
            throws ExecutorManagerException {
        return 0;
    }
    ...
    public int getUserExecutableFlowsTotalByProjectIdAndFlowId(int projectId, String flowId, int from,
                                                               int length, List<ExecutableFlow> outputList, String userName)
            throws ExecutorManagerException {
        return 0;
    }
    ...
    @Override
    public int getUserExecHistoryQuickSerachTotal(Map<String, String> filterMap)
            throws ExecutorManagerException {
        return 0;
    }
    ...
    @Override
    public List<ExecutableFlow> getTodayExecutableFlowData(String userName)
            throws ExecutorManagerException {
        return null;
    }

    @Override
    public List<ExecutableFlow> getTodayExecutableFlowDataNew(String userName)
            throws ExecutorManagerException {
        return null;
    }

    @Override
    public Integer getTodayFlowRunTimesByFlowId(String projectId, String flowId, String usename)
            throws ExecutorManagerException {
        return null;
    }

    @Override
    public List<ExecutableFlow> getRealTimeExecFlowData(String userName)
            throws ExecutorManagerException {
        return null;
    }
    ...
    @Override
    public List<ExecutionCycle> listExecutionCycleFlows(Optional<String> usernameOP, int offset,
                                                        int length)
            throws ExecutorManagerException {
        return null;
    }
    ...
    @Override
    public List<ExecutionCycle> listExecutionCycleFlows(String username, List<Integer> projectIds,
    ...
    @Override
    public ExecutionCycle getExecutionCycleFlow(String projectId, String flowId)
            throws ExecutorManagerException {
        return null;
    }
