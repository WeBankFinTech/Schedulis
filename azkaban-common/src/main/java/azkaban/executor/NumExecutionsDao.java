    ...
    public int fetchNumExecutableNodes(final int projectId, final String jobId, String nestedId)
        ...
        try {
            ...
            if (StringUtils.isNotEmpty(nestedId)) {
                ...
                String sql = "SELECT COUNT(1) FROM execution_jobs WHERE project_id=? AND job_id=?";
                ...
                if (split.length > 1) {
                    for (int i = 0; i < split.length - 1; i++) {
                        if (StringUtils.isNotEmpty(split[i])) {
                            conditionSql = conditionSql + "and flow_id like ? ";
              ...
              return this.dbOperator.query(sql, intHandler, objects);
            } else {
