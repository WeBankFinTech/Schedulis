 ...
 * @author WTSS
 */
public abstract class AbstractCandidateFilter<T, V> {

  protected static final Logger logger = LoggerFactory.getLogger(AbstractCandidateFilter.class);
  ...
  public boolean filterTarget(final T filteringTarget, final V referencingObject) {
    ...
    for (final FactorFilter<T, V> filter : filterList) {
      ...
      if (!result) {
        logger.info(String.format("[Factor: %s] filter '%s' result : %s ",
            filter.getFactorName(), filteringTarget == null ? "(null)" : filteringTarget.toString(),
            false));
        break;
      }
    }
