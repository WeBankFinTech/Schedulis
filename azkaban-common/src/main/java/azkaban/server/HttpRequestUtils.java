...
public class HttpRequestUtils {
    ...
    public static ExecutionOptions parseFlowOptions(ExecutableFlow executableFlow, final HttpServletRequest req)
        ...
        if (hasParam(req, ExecutionOptions.DISABLE)) {
            ...
            if (!disabled.isEmpty()) {
                ...
                // 判断 disabled job 的 auto.disabled 属性是否为 true，如果为 true 则记录到新的集合中
                ArrayList<Object> autoDisabledJobs = new ArrayList<>();
                FlowUtils.applyAutoDisabledJobs(disabledList, executableFlow, autoDisabledJobs);
                List<Object> newDisabledList = new ArrayList<>();
                if (hasParam(req, Constants.DISABLED_FLAG)) {
                    String ableFlag = req.getParameter(Constants.DISABLED_FLAG);
                    if (ableFlag.equals("noChild")) {
                        addNoChild(disabledList, newDisabledList);

                    } else {
                        newDisabledList.addAll(disabledList);
                    }
                ...
                execOptions.setDisabledJobs(newDisabledList);
                execOptions.setAutoDisabledJobs(autoDisabledJobs);
            }
            ...
            if (!enabled.isEmpty()) {
                ...
                if (hasParam(req, Constants.ENABLE_FLAG)) {
                    String ableFlag = req.getParameter(Constants.ENABLE_FLAG);
                    enabledList.add(ableFlag);
                }
    ...
    private static void addNoChild(List<Object> list, List<Object> newList) {
        for (Object job : list) {
            String[] split = job.toString().split(":");
            int last = split.length - 1;
            String lastJob = split[last];
            newList.add(lastJob);
        }
    }
    ...
    public static ExecutionOptions parseFlowOptions(ExecutableFlow executableFlow, final JsonObject jsonObject)
            throws ServletException {
        final ExecutionOptions execOptions = new ExecutionOptions();
        ...
        if (jsonObject.has(ExecutionOptions.DISABLE)) {
            ...
            if (disabledList != null) {
                List<Object> newDisabledList = new ArrayList<>();
                if (jsonObject.has(Constants.DISABLED_FLAG)) {
                    String ableFlag = jsonObject.get(Constants.DISABLED_FLAG).getAsString();
                    if (ableFlag.equals("noChild")) {
                        addNoChild(disabledList, newDisabledList);
        ...
        if (jsonObject.has(ExecutionOptions.ABLE)) {
            List<Object> enabledList = GsonUtils.jsonToJavaObject(jsonObject.get(ExecutionOptions.ABLE),
                    new TypeToken<List<Object>>() {
                    }.getType());

            if (enabledList != null) {
                if (jsonObject.has(Constants.ENABLE_FLAG)) {
                    String ableFlag = jsonObject.get(Constants.ENABLE_FLAG).getAsString();
                    enabledList.add(ableFlag);
                }
                execOptions.setDisabledJobs(enabledList);
            }
        ...
        if (jsonObject.has(ExecutionOptions.ENABLED_CACHE_PROJECT_FILES)) {
            execOptions.setEnabledCacheProjectFiles(
                    jsonObject.get(ExecutionOptions.ENABLED_CACHE_PROJECT_FILES).getAsBoolean());
        } else {
...
//    }
    ...
    public static Map<String, Object> parseWebOptions(final ExecutionOptions flowOptions)
        ...
        responseMap.put(AUTO_DISABLED_JOBS, flowOptions.getAutoDisabledJobs());
