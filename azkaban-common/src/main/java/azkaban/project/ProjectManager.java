...
public class ProjectManager {
                    ...
                    try {
                        ...
                        if (null != wtssUser) {
                            ...
                            if (StringUtils.isNotBlank(userEmail)) {
                                ...
                                // 用户邮箱信息为空，根据用户名进行拼接
                                alertUsers.append(wtssUser.getUsername()).append("@xxx.com")
                                        .append(";");
                            }
                        }
    ...
    public void updateProjectCreateUser(final Project project, String newCreateUser, User user) throws Exception {
        ...
        this.projectLoader.postEvent(project, EventType.CREATE_USER,
                user.getUserId() + (StringUtils.isEmpty(user.getNormalUser()) ? "" : ("(" + user.getNormalUser() + ")")), "createUser: " + oldCreateUser + " changed to " + newCreateUser);
    ...
    public void updateProjectPrincipal(final Project project, String principal, User user) throws Exception {
        ...
        this.projectLoader.postEvent(project, EventType.PRINCIPAL,
                user.getUserId() + (StringUtils.isEmpty(user.getNormalUser()) ? "" : ("(" + user.getNormalUser() + ")")), "product principal : " + oldPrincipal + " changed to " + principal);
        //重新存入缓存
    ...
    public synchronized Project purgeProject(final Project project, final User deleter)
        ...
        this.projectLoader
                .postEvent(project, EventType.PURGE, deleter.getUserId() + (StringUtils.isEmpty(deleter.getNormalUser()) ? "" : ("(" + deleter.getNormalUser() + ")")), String
    ...
    public synchronized Project removeProject(final Project project, final User deleter)
        ...
        this.projectLoader.postEvent(project, EventType.DELETED, deleter.getUserId() + (StringUtils.isEmpty(deleter.getNormalUser()) ? "" : ("(" + deleter.getNormalUser() + ")")),
                null);
    ...
    public void restoreProject(String projectName, int projectId, User user, HashMap<String, Object> ret) throws ProjectManagerException {
        ...
        this.projectLoader.postEvent(p, EventType.RESTORE_PROJECT, user.getUserId() + (StringUtils.isEmpty(user.getNormalUser()) ? "" : ("(" + user.getNormalUser() + ")")), "restore project.");
        if (this.isHaModel) {
            HttpUtils.reloadWebData(this.getProps().getStringList("azkaban.all.web.url"), "reloadProject",
                    p.getName());
        }
    ...
    public void updateProjectDescription(final Project project, final String description,
        ...
        this.projectLoader.postEvent(project, EventType.DESCRIPTION,
                modifier.getUserId() + (StringUtils.isEmpty(modifier.getNormalUser()) ? "" : ("(" + modifier.getNormalUser() + ")")), "Description changed to " + description);
        if (this.isHaModel) {
            HttpUtils.reloadWebData(this.getProps().getStringList("azkaban.all.web.url"), "reloadProject",
                    project.getName());
        }
    ...
    public void updateJobLimit(final Project project, final int jobLimit,
        ...
        this.projectLoader.postEvent(project, EventType.JOB_EXECUTE_LIMIT,
                modifier.getUserId() + (
                        StringUtils.isEmpty(modifier.getNormalUser()) ? ""
    ...
    public ProjectVersion getProjectVersion(Project project, int version) {
        List<ProjectVersion> projectVersionList = this.projectLoader.getProjectVersion(project,
            version);
        if (projectVersionList != null && !projectVersionList.isEmpty()) {
            return projectVersionList.get(0);
        }
    ...
    public void setJobOverrideProperty(final Project project, final Flow flow, final Props prop,
        ...
        this.projectLoader.postEvent(project, EventType.PROPERTY_OVERRIDE,
                modifier.getUserId() + (StringUtils.isEmpty(modifier.getNormalUser()) ? "" : ("(" + modifier.getNormalUser() + ")")), diffMessage);
        return;
    }
    ...
    public void addProjectProxyUser(final Project project, final String proxyName,
        ...
        this.projectLoader.postEvent(project, EventType.PROXY_USER,
                modifier.getUserId() + (StringUtils.isEmpty(modifier.getNormalUser()) ? "" : ("(" + modifier.getNormalUser() + ")")), "Proxy user " + proxyName
                        + " is added to project.");
    ...
    public void updateProjectPermission(final Project project, final String name,
        ...
        if (group) {
            this.projectLoader.postEvent(project, EventType.GROUP_PERMISSION,
                    modifier.getUserId() + (StringUtils.isEmpty(modifier.getNormalUser()) ? "" : ("(" + modifier.getNormalUser() + ")")), "Permission for group " + name + " set to "
                            + perm.toString());
        } else {
            this.projectLoader.postEvent(project, EventType.USER_PERMISSION,
                    modifier.getUserId() + (StringUtils.isEmpty(modifier.getNormalUser()) ? "" : ("(" + modifier.getNormalUser() + ")")), "Permission for user " + name + " set to "
                            + perm.toString());
        }
        if (this.isHaModel) {
    ...
    public void removeProjectPermission(final Project project, final String name,
        ...
        if (group) {
            this.projectLoader.postEvent(project, EventType.GROUP_PERMISSION,
                    modifier.getUserId() + (StringUtils.isEmpty(modifier.getNormalUser()) ? "" : ("(" + modifier.getNormalUser() + ")")), "Permission for group " + name + " removed.");
        } else {
            this.projectLoader.postEvent(project, EventType.USER_PERMISSION,
                    modifier.getUserId() + (StringUtils.isEmpty(modifier.getNormalUser()) ? "" : ("(" + modifier.getNormalUser() + ")")), "Permission for user " + name + " removed.");
        }
    ...
    public Project createProject(final String projectName, final String description, final String group,
        ...
        this.projectLoader.postEvent(newProject, EventType.CREATED, creator.getUserId() + (StringUtils.isEmpty(creator.getNormalUser()) ? "" : ("(" + creator.getNormalUser() + ")")),
                null);
    ...
    public void removeProjectPermission(final Project project, final String userId, final User modifier) throws ProjectManagerException {
        ...
        this.projectLoader.postEvent(project, EventType.USER_PERMISSION,
                modifier.getUserId() + (StringUtils.isEmpty(modifier.getNormalUser()) ? "" : ("(" + modifier.getNormalUser() + ")")), "Permission for user " + userId + " removed.");
        if (this.isHaModel) {
            HttpUtils.reloadWebData(this.getProps().getStringList("azkaban.all.web.url"),
                    "refreshProjectPermission", project.getName());
        }
