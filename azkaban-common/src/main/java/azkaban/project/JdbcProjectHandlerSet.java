...
class JdbcProjectHandlerSet {
    ...
    public static class ProjectVersionEntityResultHandler implements
        ResultSetHandler<List<ProjectVersion>> {

        public static String SELECT_PROJECT_VERSION =
            "SELECT project_id, version, upload_time FROM project_versions WHERE project_id = ? and version = ? and num_chunks != 0 ";
        ...
        public List<ProjectVersion> handle(final ResultSet rs) throws SQLException {
            ...
            final List<ProjectVersion> resultList = new ArrayList<>();
            do {
                final int projectId = rs.getInt(1);
                final int version = rs.getInt(2);
                final long uploadTime = rs.getLong(3);
