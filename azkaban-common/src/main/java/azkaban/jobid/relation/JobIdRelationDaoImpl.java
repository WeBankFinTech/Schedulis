...
public class JobIdRelationDaoImpl implements JobIdRelationDao {
  ...
  private static final String GET_RELATION_BY_APPID =  "SELECT w.id FROM wtss_job_id_relation w " +
          " WHERE w.`application_id` = ? limit 1";
  ...
  static class FetchJobIdRelationId implements ResultSetHandler<List<JobIdRelation>> {
  ...
  @Override
  public JobIdRelation getJobIdRelationByAppId(String appId) throws SQLException {
    List<JobIdRelation> query = dbOperator.query(GET_RELATION_BY_APPID, new FetchJobIdRelationId(), appId);
    if (CollectionUtils.isNotEmpty(query)) {
      return query.get(0);
    } else {
      return null;
