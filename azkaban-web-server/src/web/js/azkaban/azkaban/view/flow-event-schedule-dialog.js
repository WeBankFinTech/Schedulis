...
azkaban.FlowEventScheduleDialogView = Backbone.View.extend({
    ...
    getExecutionOptionData: function() {
        ...
        // 重跑白名单
        var whiteFlows = $('#job-event-schedule-failed-retry-whitelist-select').val().join(',')
    ...
    getFlowRealJobList: function(flowId, projectName) {
            ...
            var successHandler = function(data) {
                if (data.error) {
                    ...
                    eventJobScheduleFailedRetryWhitelistView.model.set({ "jobList": data.jobList });
        ...
        var successHandler = function(data) {
            if (data.error) {
                ...
                if (whiteFlows) {
                    // 延迟执行，防止页面为渲染时设置值无效
                    setTimeout(function() {
                        eventJobScheduleFailedRetryWhitelistView.setHasSelectJob(whiteFlows.split(','), jobFailedRetryOptions)
                    }, 500)
                } else{
                    // 清空白名单列表
                    eventJobScheduleFailedRetryWhitelistView.handleFailedRetryListChange()
                }
...
$(function() {
    ...
    jobEventScheduleRetryWhitelistModel = new azkaban.JobEventScheduleRetryWhitelistModel()
    eventJobScheduleFailedRetryWhitelistView = new azkaban.EventJobScheduleFailedRetryWhitelistView({
        el: $('#job-event-schedule-failed-retry-whitelist-view'),
        model: jobEventScheduleRetryWhitelistModel,
    });
...
//用于保存浏览数据，切换页面也能返回之前的浏览进度。
var jobEventScheduleRetryWhitelistModel;
azkaban.JobEventScheduleRetryWhitelistModel = Backbone.Model.extend({});

var eventJobScheduleFailedRetryWhitelistView 
azkaban.EventJobScheduleFailedRetryWhitelistView= Backbone.View.extend({
    events: {
        "select2:opening #job-event-schedule-failed-retry-whitelist-select": "loadSubJobList"
    },

    initialize: function(setting) {
        $("#job-event-schedule-failed-retry-whitelist-select").select2({
            width:'100%',
            placeholder: wtssI18n.view.selectTaskPro, //默认文字提示
            placeholder: '请选择',
            allowClear: true,
    ...
    handleFailedRetryListChange: function(){
        const whiteListSelect = $('#job-event-schedule-failed-retry-whitelist-select');
