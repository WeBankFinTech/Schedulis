...
$(function () {
    ...
    // 获取正在运行
    function getRunningList(type = 'batchReRun') {
        $.ajax({
            url: "/executor",
            dataType: "json",
            contentType: "application/json; charset=utf-8",
            ...
            success: function (data) {
                ...
                if(type === 'batchReRun'){
                    validFlowPermission();
                } else if (type === 'batchHisoryReRun'){
                    flowExecuteDialogView.show(window.reScheduleCheckedtList,'batchHisoryReRun')
                    console.log(window.reScheduleCheckedtList)
                }
            }
        });
    }
    ...
    $('#batch-reRun-btn').click(function () {
       ...
       for(var i in checkboxList){
           ...
           if ( !isNaN(+i) && currentDom.checked) {
            ...
            var status = currentDom.getAttribute('status')
            if(['FAILED','KILLED'].indexOf(status) === -1){
                messageBox.show(wtssI18n.view.reRunFlowShouldBeFailed,'warning');
                return
            }
    ...
    // 批量历史重跑
    $('#batch-history-reRun-btn').click(function () {
        var checkboxList = $("#executingJobs .reSchedule-checkbox");
        // 获取参数，并通过project、flowid去重
        for(var i in checkboxList){
            var currentDom = checkboxList[i];
        ...
        // 调用正在运行接口，校验工作流是否正在运行
        getRunningList('batchHisoryReRun');
    });
    // 批量导出
...
azkaban.HistoryListView = Backbone.View.extend({
    ...
    render: function () {
        ...
        for (var i = 0; i < historyList.length; ++i) {
            ...
            $(checkbox).attr("type", "checkbox").attr("class", "reSchedule-checkbox").attr("projectName",  historyList[i].projectName).attr("projectId", historyList[i].projectId).attr("flowId", historyList[i].flowId).attr("status",historyList[i].status).attr("value", historyList[i].executionId);;
