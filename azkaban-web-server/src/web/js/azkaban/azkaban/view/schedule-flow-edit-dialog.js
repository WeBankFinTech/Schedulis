...
azkaban.FlowScheduleDialogView = Backbone.View.extend({
    ...
    getExecutionOptionData: function() {
        ...
        // 重跑白名单
        var whiteFlows = $('#job-schedule-failed-retry-whitelist-select').val().join(',')
    ...
    getFlowRealJobList: function(flowId, projectName) {
        ...
        var successHandler = function(data) {
            if (data.error) {
                ...
                jobScheduleFailedRetryWhitelistView.model.set({ "jobList": data.jobList });
    ...
    loadScheduleRunningInfo: function(data) {
        ...
        var successHandler = function(data) {
            if (data.error) {
                ...
                var whiteFlows = data.schedule.otherOptions.whiteFlows
                ...
                // 失败重试白名单数据回填
                if (whiteFlows) {
                    console.log(whiteFlows,'whiteFlows')
                    // 延迟执行，防止页面为渲染时设置值无效
                    setTimeout(function() {
                        jobScheduleFailedRetryWhitelistView.setHasSelectJob(whiteFlows.split(','), jobFailedRetryOptions)
                    }, 500)
                } else {
                    // 清空白名单列表
                    jobScheduleFailedRetryWhitelistView.handleFailedRetryListChange()
                }
...
$(function() {
    ...
    jobScheduleRetryWhitelistModel = new azkaban.JobScheduleRetryWhitelistModel();

    jobScheduleFailedRetryWhitelistView = new azkaban.JobScheduleFailedRetryWhitelistView({
        el: $('#job-schedule-failed-retry-whitelist-view'),
        model: jobScheduleRetryWhitelistModel,
    });
...
//用于保存浏览数据，切换页面也能返回之前的浏览进度。
var jobScheduleRetryWhitelistModel;
azkaban.JobScheduleRetryWhitelistModel = Backbone.Model.extend({});

var jobScheduleFailedRetryWhitelistView 
azkaban.JobScheduleFailedRetryWhitelistView= Backbone.View.extend({
    events: {
        "select2:opening #job-schedule-failed-retry-whitelist-select": "loadSubJobList"
    },

    initialize: function(setting) {
        $("#job-schedule-failed-retry-whitelist-select").select2({
            width:'100%',
            placeholder: wtssI18n.view.selectTaskPro, //默认文字提示
            placeholder: '请选择',
            allowClear: true,
    ...
    handleFailedRetryListChange: function(){
        const whiteListSelect = $('#job-schedule-failed-retry-whitelist-select');
        whiteListSelect.data('initial-loaded', false);
        whiteListSelect.empty()
    },

    loadSubJobList: function(e){
        var executingData = flowScheduleDialogView.model.get('executingData');
    ...
    setHasSelectJob: function (selecteds, jobFailedRetryOptions){
        var executingData = flowScheduleDialogView.model.get('executingData');
        const superFlows = jobFailedRetryOptions ? jobFailedRetryOptions.map(item=>item.jobName.split(',').shift()).join(',') : ''
        console.log(superFlows, 'superFlows')
        const whiteListSelect = $('#job-schedule-failed-retry-whitelist-select');
        $.ajax({
            url: "/manager?project=" + executingData.project,
            ...
            success: function(data) {
                ...
                whiteListSelect.trigger('change');
                whiteListSelect.val(selecteds).trigger('change');
