...
$(function() {
    ...
    function settingBatchAlertTab(type) {
        ...
        $('#batchSlaDutyGroup').val(null).trigger('change')
        $('#batchUpdateSlaDutyGroup').val(null).trigger('change')
    ...
    $.ajax({
        ...
        error: function (error) {
            ...
            messageBox.show(responseText.message || error, 'danger');
            console.log('error', error)
        },
        success: function (data) {
            var dutyGroups = []
            if (data.dutyGroupList){
                dutyGroups = data.dutyGroupList.map(item=>({
                    ...item,
                    id: item.id,
                    text: item.groupName
                }))
            ...
            $('#batchSlaDutyGroup').val(null).trigger('change')
            $('#batchUpdateSlaDutyGroup').select2({
                placeholder: '请选择',
                allowClear: true,
                multiple: false,
                width: '300px',
    ...
    handleBatchUpdateAlert: function () {
        ...
        var batchSlaData = {
            scheduleInfos: {
                ...
                groupId: $('#batchUpdateSlaDutyGroup').val() ? parseInt($('#batchUpdateSlaDutyGroup').val()) : null
            }
        }
...
azkaban.CycleScheduleView = Backbone.View.extend({
    ...
    render: function(evt) {
        ...
        for (var i = 0; i < scheduleList.length; ++i) {
            ...
            var editSchedBtn = document.createElement("button");
            $(editSchedBtn).attr("class", "btn btn-sm btn-success").attr("type", "button")
                .attr("onclick", "editCycleScheduleClick(" + scheduleList[i].currentExecId + ",'" + scheduleList[i].projectName + "','" + scheduleList[i].projectId + "','" + scheduleList[i].flowId + "','" + scheduleList[i].cronExpression + "','" + scheduleList[i].id + "')");
