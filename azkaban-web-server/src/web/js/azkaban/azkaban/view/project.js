...
function renderTree (el, data, prefix, filterFlowId, subFlowLevel) {//过滤job记录 subFlowLevel 层级大于2时展开
  ...
  for (var i = 0; i < nodeArray.length; ++i) {
    ...
    var nestedId = nodeArray[i].nestedId || nodeArray[i].id;
    ...
    if (job.type == "flow") {
      ...
      var iconDiv = document.createElement('div');
      $(iconDiv).addClass('icon');
      $(a).attr("href", filterXSS(requestURL + name + "&nestedId=" + nestedId));
      $(a).append(iconDiv);
    }
    ...
    if (execAccess) {
      ...
      if (job.type == "flow") {
        $(aJobHistory).attr("href", filterXSS("/manager?project=" + project + "&flow=" + job.flow + "&nestedId=" + nestedId  + "#executions"));
      } else {
        $(aJobHistory).attr("href", filterXSS("/manager?project=" + project + "&job=" + name + "&nestedId=" + nestedId + "&history"));
      }
