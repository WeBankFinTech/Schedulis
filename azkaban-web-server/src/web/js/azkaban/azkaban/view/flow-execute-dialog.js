...
function batchRepeatFlow(recoverData) {
    executeURL = "/executor?ajax=batchRepeatCollection";
    $.ajax({
        type: "POST",
...
function batchCheckRecoverParam(recoverData, repeatFun) {
    executeURL = "/executor?ajax=batchRecoverParamVerify";
    ...
    const projectsStr = recoverData.projects.map(item => "projects=" + item).join('&')
    const flowsStr = recoverData.flows.map(item => "flows=" + item).join('&')
    const {projects, flows, ...others}=recoverData
    $.get(executeURL + '&' + projectsStr + '&' + flowsStr, {
        ...others,
    }, successHandler, "json").fail(function (data) {
        $("#execute-btn").attr("disabled", false).removeClass("button-disable");
    });

}
function getBatchExecutionOptionData(list) {
    ...
    var recoverData = {
        state: state,
        alertMsg: alertMsg,
        projectIds: list.map(item => item.projectId),
        projects: list.map(item => item.projectName),
        flows: list.map(item => item.flowId),
        begin: beginTime,
        end: endTime,
        ...
        enabledCacheProjectFiles: enabledCacheProjectFiles,
        recoverNum: recoverNum,
        recoverErrorOption: recoverErrorOption,
        recoverInterval: recoverInterval,
        skipDateTimeList: skipDateTimeList,
        runDateTimeList: runDateTimeList,
        taskSize: $('#recover-Concurrent-option').val(),
        taskDistributeMethod: taskDistributeMethod,
        reRunTimeInterval: reRunTimeInterval,
        finishedAlert: $("#enable-history-recover-finished-alert").is(':checked'),
        currentVersionFlag: currentVersionFlag,
        failureAlertLevel: failureAlertLevel,
        successAlertLevel: successAlertLevel,
        historyRerunAlertLevel: historyRerunAlertLevel,
        historyRerunAlertEmails: historyRerunAlertEmails,
        executeTimeBegin: executeTimeBegin,
        executeTimeEnd: executeTimeEnd,
        useTimeoutSetting: useTimeoutSetting,
        slaEmails: slaEmails,
        reverseExecuteFlag: reverseExecuteFlag,
        settings: settings,
    };
    return recoverData;
}
...
azkaban.FlowExecuteDialogView = Backbone.View.extend({
    ...
    getExecutionOptionData: function () {
        ...
        // 重跑白名单
        var whiteFlows = $('#job-failed-retry-whitelist-select').val().join(',')
    ...
    changeFlowInfo: function () {
        var loginUser = loginUser || $(".nav.navbar-nav.navbar-right li a")[1].text;
        ...
        var notifyFailure = this.model.get("notifyFailure") || {first: true, last: false};
        ...
        var overrideFailureEmails = this.model.get("failureEmailsOverride") || false;
    ...
    show: function (data, type) {
        if (type === 'batchHisoryReRun') {
            this.changeFlowInfo()
            var loginUser = $(".nav.navbar-nav.navbar-right li a")[1].text;
            console.log(type, data)
            this.model.set({ 'type': 'batchHisoryReRun', 'batchHisoryReRunData': data })
            ...
            $("#enable-history-recover").prop('checked', true)
            $('#datetimebegin').attr('disabled', null);
            $('#datetimeend').attr('disabled', null);
            $('#executeTimeBegin').attr('disabled', null);
            $('#executeTimeEnd').attr('disabled', null);
            ...
            // $("#graph-options li").not('#history-recover-li').hide()

            // 执行失败设置
            $('#should-be-hidden-if-in-batch').hide()

            $("#show-start-five-date").prop("checked", true);
            sideMenuDialogView.menuSelect($('#history-recover-li'))
        ...
        var flowInfoId = data.flowInfoId || ''
        ...
        this.loadFlowInfo(projectName, flowId, execId, flowInfoId);
        this.initTimeoutPanel(flowId);
    ...
    loadFlowInfo: function (projectName, flowId, execId, flowInfoId) {
        console.log("Loading flow " + flowId);
        fetchFlowInfo(this.model, projectName, flowId, execId, flowInfoId);
    },
    ...
    handleExecuteFlow: function (evt) {
        ...
        if (type === 'batchHisoryReRun') {
            const batchHisoryReRunData = this.model.get('batchHisoryReRunData')
            console.log(type, batchHisoryReRunData)
            const executionData = getBatchExecutionOptionData(batchHisoryReRunData)
            if (!executionData) return
            batchCheckRecoverParam(executionData, batchRepeatFlow)
            return
        }
...
$(function () {
    ...
    jobRetryWhitelistModel = new azkaban.JobRetryWhitelistModel();

    jobFailedRetryWhitelistView = new azkaban.JobFailedRetryWhitelistView({
        el: $('#job-failed-retry-whitelist-view'),
        model: jobRetryWhitelistModel,
    });
...
azkaban.JobFailedRetryView = Backbone.View.extend({
    ...
    handleAddRetryRowCallShow: function (data, jobFailedRetryOptions, jobList) {
        jobFailedRetryWhitelistView.handleFailedRetryListChange()
        var job = "";
        if (data.job) {
            job = data.job;
        }
        ...
        idSelect.on('change', function () {
            // 给白名单select做标记，下次展开白名单list重新请求数据
            jobFailedRetryWhitelistView.handleFailedRetryListChange();
        })
...
//用于保存浏览数据，切换页面也能返回之前的浏览进度。
var jobRetryWhitelistModel;
azkaban.JobRetryWhitelistModel = Backbone.Model.extend({});

var jobFailedRetryWhitelistView
azkaban.JobFailedRetryWhitelistView = Backbone.View.extend({
    events: {
        "select2:opening #job-failed-retry-whitelist-select": "loadSubJobList"
    },

    initialize: function (setting) {
        $("#job-failed-retry-whitelist-select").select2({
            width: '100%',
            placeholder: wtssI18n.view.selectTaskPro, //默认文字提示
            placeholder: '请选择',
            allowClear: true,
            multiple: true,
            ajax: null, // 禁用 AJAX 功能
            minimumInputLength: 0, // 允许空输入
            language: 'zh-CN',
        });
    },

    handleFailedRetryListChange: function () {
        const whiteListSelect = $('#job-failed-retry-whitelist-select');
        whiteListSelect.data('initial-loaded', false);
        whiteListSelect.empty()
    },

    loadSubJobList: function (e) {
        var executingData = flowExecuteDialogView.model.get('executingData');
        console.log(executingData, 'executingData');
        let superFlows = []
        var tdFailedRetrys = document.getElementById("jobFailedRetryTable").tBodies[0];
        for (var row = 0; row < tdFailedRetrys.rows.length - 1; row++) {
            var tdFailedRetry = tdFailedRetrys.rows[row];
        ...
        superFlows = Array.from(new Set(superFlows)).join(',')
        console.log(superFlows, 'superFlows')
        const whiteListSelect = $('#job-failed-retry-whitelist-select');
        if (whiteListSelect.data('initial-loaded')) {
            return
        }
        e.preventDefault()
        // 模拟 AJAX 请求获取初始列表
        $.ajax({
            url: "/manager?project=" + executingData.project,
            data: {
                ajax: "getChildNodes",
            ...
            success: function (data) {
                const whiteList = data.jobList.map(item => ({ text: item, id: item }))
                // 将数据存储到 Select2 的 data 中
                whiteListSelect.empty(); // 清空现有选项
                for (var i = 0; i < whiteList.length; i++) {
                    var option = new Option(whiteList[i].text, whiteList[i].id, false, false);
                    whiteListSelect.append(option);
                }
                whiteListSelect.trigger('change');

                // 标记数据已加载
                whiteListSelect.data('initial-loaded', true);
                whiteListSelect.select2('open')
            },
            error: function () {
                console.error('获取初始列表失败');
            }
        });
    },
    setHasSelectJob: function (selecteds, jobFailedRetryOptions) {
        var executingData = flowExecuteDialogView.model.get('executingData');
        const superFlows = jobFailedRetryOptions ? jobFailedRetryOptions.map(item => item.split(',').shift()).join('') : ''
        console.log(superFlows, 'superFlows')
        const whiteListSelect = $('#job-failed-retry-whitelist-select');
        $.ajax({
            url: "/manager?project=" + executingData.project,
