...
function ajaxCall (requestURL, data, callback) {
  ...
  function fetchFlowInfo (model, projectName, flowId, execId, flowInfoId) {
    var fetchData = { "project": projectName, "ajax": "flowInfo", "flow": flowId , "id": flowInfoId || ''};
    if (execId) {
      ...
      if (data.error) {
        ...
        if(flowInfoId){
          model.set({
            ...
            "successEmails": data?.executionOptions?.successEmails,
            "successEmailsOverride": data?.executionOptions?.successEmailsOverride,
            "failureEmails": data?.executionOptions?.failureEmails,
            "failureEmailsOverride": data?.executionOptions?.failureEmailsOverride,
            "failureAction": data?.executionOptions?.failureAction,
            "rerunAction": data?.executionOptions?.rerunAction,
            "notifyFailure": {
              "first": data?.executionOptions?.notifyOnFirstFailure,
              "last": data?.executionOptions?.notifyOnLastFailure
            },
            "flowParams": data?.executionOptions?.flowParameters,
            "jobOutputGlobalParam": data.jobOutputGlobalParam,
            "nsWtss": data.nsWtss,
            "isRunning": data.running,
            "nodeStatus": data.nodeStatus,
            "concurrentOption": data?.executionOptions?.concurrentOptions,
            "pipelineLevel": data.pipelineLevel,
            "pipelineExecution": data.pipelineExecution,
            "queueLevel": data.queueLevel,
            "slaEmails": data?.slaOptions?.length > 0 ? data?.slaOptions[0]?.info?.EmailList.join(',') : '',
            "failureAlertLevel": data?.otherOptions?.failureAlertLevel,
            "successAlertLevel": data?.otherOptions?.successAlertLevel,
    
            "useTimeoutSetting": data?.slaOptions?.length > 0 ? true: false,
    
            "ruleType": data?.slaOptions?.length > 0 ? data?.slaOptions[0]?.type : '',
            "duration": data?.slaOptions?.length > 0 ? data?.slaOptions[0]?.info?.TimeSet : '',
            "emailAction":data?.slaOptions?.length > 0 ? data?.slaOptions[0]?.info?.email_action_set : '',
            "killAction": data?.slaOptions?.length > 0 ? data?.slaOptions[0]?.info?.kill_flow_action_set: '',
            "slaAlertLevel": data?.slaOptions?.length > 0 ? data?.slaOptions[0]?.level : '',
            "slaAlertType": data?.slaOptions?.length > 0 ? data?.slaOptions[0]?.info?.SlaAlertType : '',
    
            "jobFailedRetryOptions": data?.otherOptions?.jobFailedRetryOptions,
            "jobSkipFailedOptions": data?.otherOptions?.jobSkipFailedOptions,
            "jobSkipActionOptions": data?.otherOption?.jobSkipActionOptions,
    
            "flowRetryAlertOption": data?.otherOption?.flowRetryAlertOption,
            "flowType": data.flowType,
            "enabledCacheProjectFiles": data?.executionOptions?.enabledCacheProjectFiles,
          });
        } else{
          model.set({
            "jobDisabled": data.disabled,
