...
var nodeClickCallback = function(event, model, node) {
        ...
        //获取节点最新信息
        // http://:8290/executor?execid=238653&ajax=fetchexecflow&nodeNestedId=
        var newAttempt = 0;
        ...
        var requestURL = filterXSS("/manager?project=" + projectName + "&flow=" +flowId + "&job=" + tmpId + "&nestedId=" + (node.nestedId || node.id));
        var menu = [];

        var jobHistoryURL = filterXSS("/manager?project=" + projectName + "&job=" + node.id + "&nestedId=" + (node.nestedId || node.id) + "&history");
...
var jobClickCallback = function(event, model, node, isSearchJob = false) {
    ...
    if (type == "flow") {
        //打开工作流,需要显示当前执行结果,需要execid,同时也需要当前节点
        var flowRequestURL = filterXSS("/manager?project=" + projectName + "&flow=" + node.flowId + "&nestedId=" + (node.nestedId || node.id));
        //如果是执行记录,则显示执行状态,如果是工作流,则显示工作流
        if (executorId) {
            flowRequestURL = filterXSS("/executor?execid=" + executorId +
                "&nodeNestedId=" + node.nestedId);
...
var nodeDBClickCallback = function(event, model, node) {
    ...
    if (node.flowId) {
        var requestURL = filterXSS("/manager?project=" + projectName + "&flow=" + node.flowId + "&job=" + jobId + "&treeFlow=" + flowId + "&nestedId=" + (node.nestedId || node.id));
        window.open(requestURL);
    } else {
        var requestURL = filterXSS("/manager?project=" + projectName + "&flow=" + flowId + "&job=" + jobId + "&nestedId=" + (node.nestedId || node.id));
        window.open(requestURL);
    }
