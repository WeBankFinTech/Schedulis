...
azkaban.JobListView = Backbone.View.extend({
    ...
    filterJobs: function(self) {
        var filter = this.filterInput.val().toLowerCase();
        ...
        for (var key in this.listNodes) {
            ...
            var index = nodeName.toLowerCase().indexOf(filter);
            if (index == -1) {
                continue;
            }
            ...
            var newHTML = filterXSS(nodeName.substring(0, index) +
                "<span class=\"filterHighlight\">" +
                nodeName.substring(index, endIndex) + "</span>" +
                nodeName.substring(endIndex, nodeName.length), { 'whiteList': { 'span': ['class'] } });
    ...
    render: function(self) {
        ...
        this.changeStatuses(data);
        var urlParam = getUrlQuery(window.location.search)
        if (urlParam.jobId) {
            var selectJob = $("#joblist li[jobId='" + urlParam.jobId + "']")
            selectJob[0] && selectJob[0].click()
            $("#open-joblist-btn").click()
        }
    },
    ...
    renderTree: function(el, data, prefix) {
        ...
        for (var i = 0; i < nodeArray.length; ++i) {
            ...
            $(li).addClass("listElement tree-list-item").attr('jobId', nodeArray[i].id);
