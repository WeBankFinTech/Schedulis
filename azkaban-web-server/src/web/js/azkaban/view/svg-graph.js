...
azkaban.SvgGraphView = Backbone.View.extend({
        ...
        for (var i = 0; i < data.nodes.length; ++i) {
            ...
            if (node.disabled) {
                if (node.gNode) {
                    ...
                    title = this.trimFlowProcessTitle(node.label, title);
    ...
    assignInitialStatus: function(evt, data) {
        ...
        for (var i = 0; i < data.nodes.length; ++i) {
            ...
            title = this.trimFlowProcessTitle(updateNode.label, title)
            if (switchingFlow) {
        ...
        if (updateData.nodes) {
            ...
            for (var i = 0; i < updateData.nodes.length; ++i) {
                ...
                if (g) {
                    ...
                    title = this.trimFlowProcessTitle(nodeToUpdate.label, title);
    ...
    trimFlowProcessTitle: function(label, title) {
        var trimFlowName = JSON.parse(sessionStorage.getItem('trimFlowName'));
        if (trimFlowName) {
            title = label + '  ' + title;
        }
