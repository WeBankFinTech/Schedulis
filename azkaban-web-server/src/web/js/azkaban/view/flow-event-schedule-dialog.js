...
azkaban.FlowEventScheduleDialogView = Backbone.View.extend({
  ...
  getExecutionOptionData: function () {
    ...
    var notifyFailureFirst = $('#event-schedule-notify-failure-first').is(':checked');
    var notifyFailureLast = $('#event-schedule-notify-failure-last').is(':checked');
  ...
  changeFlowInfo: function () {
    ...
    if (overrideSuccessEmails) {
      $('#event-schedule-override-success-emails').attr('checked', true);
    }
    else {
      $('#event-schedule-override-success-emails').attr('checked', false);
      $('#event-schedule-success-emails').attr('disabled', 'disabled');
    }
    if (overrideFailureEmails) {
      $('#event-schedule-override-failure-emails').attr('checked', true);
    }
    else {
      $('#event-schedule-override-failure-emails').attr('checked', false);
      $('#event-schedule-failure-emails').attr('disabled', 'disabled');
    }
    ...
    if (notifyFailure.first) {
      $('#event-schedule-notify-failure-first').attr('checked', true);
      $('#event-schedule-notify-failure-first').parent('.btn').addClass('active');
    }
    if (notifyFailure.last) {
      $('#event-schedule-notify-failure-last').attr('checked', true);
      $('#event-schedule-notify-failure-last').parent('.btn').addClass('active');
    }

    if (rerunAction) {
      $('input[value=' + rerunAction + '][name="rerunActionEvent"]').prop("checked", "checked");
    }
  ...
  show: function (data) {
    ...
    if (scheduleId) {
      ...
      $('#event-schedule-topic_input').html('').append(topicOption).trigger('change');
      var msgOption = new Option(data.msgName, data.msgName)
      $('#event-schedule-msgname_input').html('').append(msgOption).trigger('change');
  ...
  loadGraph: function (projectName, flowId, exgraph, callback) {
    ...
    var successHandler = function (data) {
      ...
      if (data.error) {
        ...
        $('#title-user-operator-schedule-flow-permit').text(
          wtssI18n.view.scheduleConfigPermission);
      ...
      if (data.error) {
        ...
        if (jobSkipFailedOptions) {
          ...
          var renderSkipRow = setInterval(function () {
            var data = executingSvgGraphView && executingSvgGraphView.model.get('data')
            if (data && data.flowId === window.currentEventGraphFlowId) {
  ...
  for (var i = 0; i < data.nodes.length; ++i) {
    ...
    switch (type) {
      ...
      case 'outer':
        ...
        break;
      case 'label':
        if (node.tag && node.tag.indexOf(labelNum) > -1) {
          node.disabled = disable;
          eventScheduleRecurseTreeParent(node, disable);
        }
        break;
  ...
  $.merge(menu, [
    { break: 1 },
    {
      ...
      title: wtssI18n.view.open, callback: function () {
        ...
        {
          title: wtssI18n.view.openSendingSignal, callback: function () {
            //关闭发送信息
        ...
        {
          title: wtssI18n.view.openReceivingSignal, callback: function () {
            // 关闭接口信号
        ...
        },
        {
          ...
          key: "openNode",
          title: wtssI18n.view.openLabel, callback: function () {
          },
          submenu: [
            {
              title: wtssI18n.view.openLabel + '1', callback: function () {
                eventscheduleTouchTypecheck('label', false, 1);
              }
            },
            {
              title: wtssI18n.view.openLabel + '2', callback: function () {
                eventscheduleTouchTypecheck('label', false, 2);
              }
            },
            {
              title: wtssI18n.view.openLabel + '3', callback: function () {
    ...
    },
    {
      ...
      title: wtssI18n.view.close, callback: function () {
        ...
        {
          title: wtssI18n.view.closeSendingSignal, callback: function () {
            //关闭发送信息
            sessionStorage.setItem('disableEventchecker', 'SEND')
        ...
        {
          title: wtssI18n.view.closeReceivingSignal, callback: function () {
            // 关闭接口信号
            sessionStorage.setItem('disableEventchecker', 'RECEIVE')
            eventscheduleTouchTypecheck('eventchecker', true);
          }
        ...
        {
          key: "closeNode",
          title: wtssI18n.view.closeLabel, callback: function () {
          },
          submenu: [
            {
              title: wtssI18n.view.closeLabel + '1', callback: function () {
                eventscheduleTouchTypecheck('label', true, 1);
              }
            },
            {
              title: wtssI18n.view.closeLabel + '2', callback: function () {
                eventscheduleTouchTypecheck('label', true, 2);
              }
            },
            {
              title: wtssI18n.view.closeLabel + '3', callback: function () {
