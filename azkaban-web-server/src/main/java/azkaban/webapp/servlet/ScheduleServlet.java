...
public class ScheduleServlet extends AbstractLoginAzkabanServlet {
    ...
    private Boolean executeSetSlaAction(final HttpServletRequest req, final HashMap<String, Object> ret, User user, String scheduleId,
        ...
        slaOptions.forEach(slaOption -> {
            if (hasParam(req, "groupId")) {
                slaOption.getInfo().put(SlaOption.DUTY_GROUP_ID, req.getParameter("groupId"));
            }else {
                slaOption.getInfo().put(SlaOption.DUTY_GROUP_ID, "");
            }
        ...
        this.projectManager.postProjectEvent(project, EventType.SLA,
                user.getUserId() + (StringUtils.isEmpty(user.getNormalUser()) ? "" : ("(" + user.getNormalUser() + ")")), "SLA for flow " + sched.getFlowName() + " has been added/changed.");
    ...
    private void ajaxBatchSetSlaEmail(HttpServletRequest req, HashMap<String, Object> ret, User user) {
        ...
        String groupId = "";
        try{
            if(Objects.nonNull(scheduleInfos.get("groupId"))){
                groupId = scheduleInfos.get("groupId").getAsString();
            }
        }catch (Exception e){
            logger.error("get groupId error:{}",e);
        }
        ...
        try {
                ...
                if (CollectionUtils.isNotEmpty(slaOptions)) {
                    for (SlaOption slaOption : slaOptions) {
                        if (slaOption != null) {
                            ...
                            slaOption.getInfo().put(SlaOption.DUTY_GROUP_ID, groupId);
                        }
                    }
            ...
            for (Schedule schedule : updateList) {
                ...
                this.projectManager.postProjectEvent(project, EventType.SLA,
                        user.getUserId() + (StringUtils.isEmpty(user.getNormalUser()) ? "" : ("(" + user.getNormalUser() + ")")), "SLA for flow " + schedule.getFlowName()
    ...
    private void ajaxSetSla(final HttpServletRequest req, final HashMap<String, Object> ret, final User user) {
        try {
            ...
            String dutyGroupId = getParam(req, "groupId", "");
            ...
            for (final String set : settings.keySet()) {
                ...
                if (sla != null) {
                    ...
                    if (StringUtils.isNotEmpty(dutyGroupId)) {
                        sla.getInfo().put(SlaOption.DUTY_GROUP_ID, dutyGroupId);
                    }
                    slaOptions.add(sla);
                }
            }
...
//            }
            ...
            this.scheduleManager.insertSchedule(sched);
            this.projectManager.postProjectEvent(project, EventType.SLA,
                    user.getUserId() + (StringUtils.isEmpty(user.getNormalUser()) ? "" : ("(" + user.getNormalUser() + ")")), "SLA for flow " + sched.getFlowName()
                            + " has been added/changed.");
    ...
    private void ajaxSlaInfo(final HttpServletRequest req, final HashMap<String, Object> ret,
        ...
        try {
            ...
            if (slaOptions != null && slaOptions.size() > 0) {
                ...
                String groupId = slaOptions.get(0).getInfo().get(SlaOption.DUTY_GROUP_ID) == null ? "" : slaOptions.get(0).getInfo().get(SlaOption.DUTY_GROUP_ID).toString();
                if (StringUtils.isNotEmpty(groupId)) {
                    DutyGroup group = dutyService.getDutyGroupById(Integer.parseInt(groupId));
                    if (group != null) {
                        ret.put("groupId", group.getId());
                        ret.put("groupName", group.getGroupName());
                    }
                }
    ...
    private void ajaxScheduleFlow(final HttpServletRequest req,
        ...
        try {
            flowOptions = HttpRequestUtils.parseFlowOptions(new ExecutableFlow(project, flow), req);
        ...
        this.projectManager.postProjectEvent(project, EventType.SCHEDULE,
                user.getUserId() + (StringUtils.isEmpty(user.getNormalUser()) ? "" : ("(" + user.getNormalUser() + ")")), "Schedule " + schedule.toString()
                        + " has been added.");
...
//                }
        ...
        ExecutionOptions flowOptions = null;
        try {
            flowOptions = HttpRequestUtils.parseFlowOptions(new ExecutableFlow(project, flow), json);
            final List<String> failureEmails = flowOptions.getFailureEmails();
            logger.info("update schedule failureEmails : {} ", failureEmails);
            List<WebankUser> userList = systemManager.findAllWebankUserList(null);
            if (this.checkRealNameSwitch && WebUtils.checkEmailNotRealName(failureEmails,
    ...
    private void ajaxScheduleCronFlow(final HttpServletRequest req,
        ...
        if (personalCheckSwitch && !personalPermission(user)) {
            ret.put("status", "error");
            ret.put("message", "the real name user have no permission.");
            return;
        }
        final Flow flow = project.getFlow(flowName);
        if (flow == null) {
