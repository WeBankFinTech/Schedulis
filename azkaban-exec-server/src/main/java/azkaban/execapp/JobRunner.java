...
public class JobRunner extends EventHandler implements Runnable {
    ...
    private void intellgentDiagnosis() {
        ...
        try {
            ...
            if (null != errorCodeInfo && !errorCodeInfo.isEmpty()) {
                String knUrl = this.azkabanProps.getString(ConfigurationKeys.LINKIS_KN_URL, "");
    ...
    private void doRun() {
        ...
        //前置校验
        hookContext.setHookType(HookContext.HookType.PRE_EXEC_SYS_HOOK);
        try {
            if (!this.isKilled() && !this.isSkipped()) {
                doRunHooks();
            }
        ...
        //后置job失败执行hook- 定制需求，防止其他job失败也进入
        hookContext.setHookType(HookContext.HookType.POST_FAIL_SYS_HOOK);
        String checkType = this.node.getInputProps().getString(HookConstants.JOB_SYS_HOOK_ODLCHECK_TYPE,"");
        try {
            if (Status.isFailed(finalStatus) && !this.isKilled() && !this.isSkipped()) {
                if (HookCheckType.isFail(checkType)){
                    doRunHooks();
                }else {
                    logger.info("当前job状态: {} 和用户配置的odlcheck_type: {} 不支持后置失败型Hook",finalStatus,checkType);
                }
            }
        } catch (Exception e) {
            logger.info(
                    "execId: " + executionId + ", node: " + this.node.getId() + ", 后置job失败 hook 校验失败.", e);
        }
...
//    upload job status from db
    ...
    private void restartFailedJobHandle(Status finalStatus, boolean restartFailed)
            throws Exception {
        Props inputProps = this.node.getInputProps();
        if (inputProps != null && "true".equals(inputProps.get("jobContainsWhite"))) {
