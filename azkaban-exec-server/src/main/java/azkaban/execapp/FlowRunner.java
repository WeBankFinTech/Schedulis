...
//      this.flowKilled = false;
    ...
    private void setExecutionProps(ExecutableNode node, Props props) {
        ...
        if (null != this.flow.getOtherOption().get("jobFailedRetryOptions")) {
            ...
            //假如当前节点在失败免重跑白名单，标记该节点
            Object whiteFlows = flow.getOtherOption().get("whiteFlows");
            if (whiteFlows != null) {
                String[] split = whiteFlows.toString().split(",");
                if (Arrays.asList(split).contains(node.getId())) {
                    props.put("jobContainsWhite", "true");
                }
            }
        ...
        Map<String, String> map = (Map<String, String>) this.flow.getOtherOption().get("flowFailedRetryOption");
        Map<String, String> flowFailedRetryOption = this.flow.getFlowFailedRetry().isEmpty() ? map : this.flow.getFlowFailedRetry();
...
//    this.flow.getExecutionOptions().getFlowParameters().putAll(outputProps.getFlattened());
    ...
    private void runExecutableNode(final ExecutableNode node)
        ...
        try {
            ...
            if ("datachecker".equals(node.getType()) || "eventchecker".equals(node.getType())
                || "innerjobchecker".equals(node.getType()) || "outerjobchecker".equals(
                node.getType())) {
                initCheckExecutors();
        ...
        if (null != slaJobOptionsList) {
            try {
                for (SlaOption slaOption : slaJobOptionsList) {
                    ...
                    List<String> emailList = setDutyGroup(slaOption);
                    ...
                    slaOption.getInfo().put(SlaOption.INFO_EMAIL_LIST, emailList);
                }
            } catch (Exception e) {
    ...
    private List<String> setDutyGroup(SlaOption slaOption) {
        //有值班组，在这里修改告警人列表
        List<String> emailList = (List<String>) slaOption.getInfo().get(SlaOption.INFO_EMAIL_LIST);
        //获取值班组
        Object groupObj = slaOption.getInfo().get(SlaOption.DUTY_GROUP_ID);
        if (groupObj != null && StringUtils.isNotEmpty(groupObj.toString())) {
            this.dutyService = ServiceProvider.SERVICE_PROVIDER.getInstance(DutyServiceImpl.class);

            LinkedList<String> emailLikedList = new LinkedList<>(emailList);
            int groupId = 0;
            try {
                groupId = Integer.parseInt(groupObj.toString());
            } catch (Exception e) {
                logger.error("获取GroupId:{},失败,Exception:{}", groupId, e.getMessage());
                return emailList;
            }
            List<DutyPerson> personList = dutyService.getDutyPersonByGroupId(groupId);

            if (org.apache.commons.collections4.CollectionUtils.isNotEmpty(personList)) {
                //获取当前日期，判断哪些用户需要发送告警
                String nowDate = DateUtils.formatDate(new Date(), DateUtils.FORMATTER_Y_M_D);
                personList.forEach(dutyPerson -> {
                    //当前日期是否在此人的值班范围
                    Boolean isBetweenDate = DateUtils.isBetweenDate(DateUtils.parseDate(nowDate, DateUtils.FORMATTER_Y_M_D), dutyPerson.getBeginTime(), dutyPerson.getEndTime());
                    Boolean isDutyTime = false;
                    if (StringUtils.isNotEmpty(dutyPerson.getDutyTime())) {
                        isDutyTime = dutyPerson.getDutyTime().contains(nowDate);
                    }
                    if (isBetweenDate || isDutyTime) {
                        emailLikedList.addFirst(dutyPerson.getUserName());

                    }
                });
            }
            if (org.apache.commons.collections4.CollectionUtils.isNotEmpty(emailLikedList)) {
                //利用LinkedHashSet去重，去掉重复元素的后一个
                LinkedHashSet<String> emailLinkedSet = new LinkedHashSet<>(emailLikedList);
                slaOption.getInfo().put(SlaOption.INFO_EMAIL_LIST, emailLinkedSet.stream().collect(Collectors.toList()));
            }
