...
例： （在***REMOVED***，***REMOVED***部署1.5.0版本）：
# 在需要部署 Executor 的节点上运行以下脚本
sudo sh 1.hdp_schedulis_deploy_exec.sh ***REMOVED***:bdphdp02jobs05，***REMOVED***:bdphdp02jobs04 1.5.0
```
...
例： （初始化部署1.5.0版本数据库）：
sudo sh 3.hdp_schedulis_deploy_script.sh 1.5.0 ***REMOVED*** 3306 schedulisdb root 123456
```
