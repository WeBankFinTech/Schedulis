...
public class HadoopJavaJob extends JavaProcessJob {
  ...
  public HadoopJavaJob(String jobid, Props sysProps, Props jobProps, Logger log)
    ...
    shouldProxy =
        getSysProps().getBoolean(AbstractHadoopSecurityManager.ENABLE_PROXYING, false);
    getJobProps().put(AbstractHadoopSecurityManager.ENABLE_PROXYING,
        Boolean.toString(shouldProxy));
    obtainTokens =
        getSysProps().getBoolean(AbstractHadoopSecurityManager.OBTAIN_BINARY_TOKEN,
            false);
    noUserClasspath =
