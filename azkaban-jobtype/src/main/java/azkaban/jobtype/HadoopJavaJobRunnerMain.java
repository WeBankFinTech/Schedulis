...
public class HadoopJavaJobRunnerMain {
  ...
  public HadoopJavaJobRunnerMain() throws Exception {
    ...
    try {
      ...
      logger = LoggerFactory.getLogger(HadoopJavaJobRunnerMain.class);
      ...
      if (shouldProxy(props)) {
        ...
        if (securityEnabled) {
          ...
          logger.info("Security enabled is "
              + UserGroupInformation.isSecurityEnabled());

          logger.info("Setting mapreduce.job.credentials.binary to "
              + filelocation);
      ...
      if (shouldProxy(props)) {
        javaObject =
            getObjectAsProxyUser(props, logger, jobName, className, proxyUser);
      } else {
      ...
      logger.info("Got object " + javaObject.toString());
      ...
      if (shouldProxy(props)) {
        logger.info("Proxying enabled.");
        runMethodAsUser(props, javaObject, runMethod, proxyUser);
      } else {
      ...
      try {
        ...
        if (outputGendProps != null) {
          ...
          logger.info(GET_GENERATED_PROPERTIES_METHOD
  ...
  private void outputGeneratedProperties(Props outputProperties) {
    logger.info("Outputting generated properties to "
  ...
  public boolean shouldProxy(Properties props) {
    String shouldProxy =
        props.getProperty(AbstractHadoopSecurityManager.ENABLE_PROXYING);
