...
public class StatsUtils {
  ...
  public static Properties getJobConf(RunningJob runningJob) {
    try {
      ...
      logger.warn("Error while retrieving job conf: ", e);
    }
    return null;
  }
  ...
  public static Properties getJobConf(Configuration conf) {
    ...
    try {
      ...
      for (Map.Entry<String, String> entry : conf) {
        if ("pig.mapPlan".equals(entry.getKey())
            || "pig.reducePlan".equals(entry.getKey())) {
      ...
      logger.warn("Error while reading job conf: ", e);
    }
