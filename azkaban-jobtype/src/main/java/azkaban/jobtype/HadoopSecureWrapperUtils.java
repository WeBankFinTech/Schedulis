...
public class HadoopSecureWrapperUtils {
  ...
  private static UserGroupInformation createSecurityEnabledProxyUser(String userToProxy, String filelocation, Logger log
    ...
    log.info("Found token file.  Setting " + AbstractHadoopSecurityManager.MAPREDUCE_JOB_CREDENTIALS_BINARY
            + " to " + filelocation);
    System.setProperty(AbstractHadoopSecurityManager.MAPREDUCE_JOB_CREDENTIALS_BINARY, filelocation);
  ...
  public static UserGroupInformation setupProxyUser(Properties jobProps,
    ...
    try {
      ...
      userToProxy = jobProps.getProperty(AbstractHadoopSecurityManager.USER_TO_PROXY);
      if (securityEnabled) {
        proxyUser =
  ...
  public static boolean shouldProxy(Properties props) {
    String shouldProxy = props.getProperty(AbstractHadoopSecurityManager.ENABLE_PROXYING);
