...
$(function () {
  ...
  systemManagerModel = new azkaban.SystemManagerModel();

  systemView = new azkaban.SystemView({
    el: $('#system-options')
  });

  addSystemUserView = new azkaban.AddSystemUserView({
    el: $('#add-system-user-panel'),
    model: systemManagerModel
  });
    ...
    var successHandler = function (data) {
      if (data.error) {
        ...
        for (var i = 0; i < depList.length; i++) {
          ...
          optionHtml += "<option value='" + department.dpId + "'>" + department.dpName + "</option>"
        }
        optionHtml = filterXSS(optionHtml, { 'whiteList': { 'option': ['value'] } })
        $('#webank-department-select2').append(optionHtml);
      }
    }
  ...
  loadWebankDepartmentData: function () {
    ...
    var successHandler = function (data) {
      if (data.error) {
        ...
        optionHtml = filterXSS(optionHtml, { 'whiteList': { 'option': ['value'] } })
        $('#update-wtss-department-select').append(optionHtml);
      }
    }
