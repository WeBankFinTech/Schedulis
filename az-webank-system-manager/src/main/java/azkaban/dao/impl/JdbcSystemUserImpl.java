...
public class JdbcSystemUserImpl implements SystemUserLoader {
    ...
    @Override
    public List<WtssRole> getAllWtssRole() throws SystemUserManagerException {
        try {
            return this.dbOperator.query(WtssRoleHandler.BASE_SQL_FIND_WTSS_ROLE, new WtssRoleHandler());
        } catch (final SQLException e) {
            throw new SystemUserManagerException("Failed to get All WTSS Role", e);
        }
    }
    ...
    @Override
    public List<WtssPermissions> getAllWtssPermissions() throws SystemUserManagerException {
        try {
            return this.dbOperator.query(WtssPermissionsHandler.BASE_SQL_FIND_WTSS_PERM,
                    new WtssPermissionsHandler());
        } catch (final SQLException e) {
            throw new SystemUserManagerException("Failed to get All WTSS Permission", e);
        }
    }
    ...
    public int getWebankDepartmentTotal(final String searchterm) throws SystemUserManagerException {
        ...
        try {
            ...
            if (StringUtils.isNotBlank(searchterm)) {
                querySQL += " WHERE dp_ch_name LIKE ? ";
    ...
    public List<WebankDepartment> findAllWebankDepartmentPageOrSearch(String searchName, int pageNum, int pageSize) throws SystemUserManagerException {
        ...
        try {
            ...
            if (StringUtils.isNotBlank(searchName)) {
                querySQL += " WHERE (c.dp_ch_name LIKE ? or c.dp_name LIKE ?)";
                params.add('%' + searchName + '%');
                params.add('%' + searchName + '%');
            }
    ...
    private static class WebankDepartmentHandler implements ResultSetHandler<List<WebankDepartment>> {
        ...
        private static final String FIND_WEBANK_DEPARTMENT_NO_ORG = BASE_SQL_FIND_ALL_WEBANK_DEPARTMENT + " WHERE pid=" + Constants.WEBANK_DEPARTMENT_ID_NO;
    ...
    @Override
    public List<WtssUser> getAllWtssUser() throws SystemUserManagerException {
        try {
            return this.dbOperator.query(SystemUserHandler.BASE_SQL_FIND_WTSS_USER, new SystemUserHandler());
        } catch (final SQLException e) {
            throw new SystemUserManagerException("Failed to Get All WTSS User.", e);
        }
    }
    ...
    @Override
    public Map<Long, List<Integer>> getDepartmentExecutor(List<Long> dpId) throws SystemUserManagerException {
