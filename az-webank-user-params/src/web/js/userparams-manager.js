...
azkaban.SystemDepartmentGroupView = Backbone.View.extend({
  ...
  handlePageChange: function (evt) {
    var requestURL = "/userparams?ajax=fetchAllUserVariable";
...
azkaban.AddDepartmentGroupView = Backbone.View.extend({
  ...
  handleAddDepartment: function (evt) {
    ...
    var requestUrl = "/userparams?ajax=addUserVariable";
    var model = this.model;
    var successHandler = function (data) {
      if (data.error) {
...
azkaban.UpdateDepartmentGroupView = Backbone.View.extend({
  ...
  getWtssUsers: function () {
    ...
    var successHandler = function (data) {
      if (data.error) {
        ...
        model.set("wtssuserList", data.webankUserList);
      }
    }
  ...
  handleAddExecutor: function (userName) {
    ...
    $(remove).append(removeBtn);
    cDelete.appendChild(remove);
    this.loadWebankUserData(idSelect);
    if(userName){
      $(idSelect).val(userName).select2();
    }
  },
  ...
  handleUpdateSystemDepartment: function (evt) {
    ...
    var id = $('#old-id').val();
    var requestUrl = "/userparams?ajax=updateUpdateUserVariable&oldKeyName=" + $('#old-key-name').val();
  ...
  handleDeleteSystemDepartment: function (evt) {
    deleteDialogView.show(wtssI18n.deletePro.deleteUserParameters, wtssI18n.deletePro.whetherDeleteUserParameters, wtssI18n.common.cancel, wtssI18n.common.delete, '', function() {
        console.log("Delete user param Department button.");
        var id = $('#old-id').val();
        var requestUrl = "/userparams?ajax=deleteUserVariable";
        var model = this.model;
        var successHandler = function (data) {
            if (data.error) {
  ...
  loadDepartmentData: function () {
    ...
    var requestURL = "/userparams?ajax=fetchUserVariableById";
    ...
    var successHandler = function (data) {
      ...
      if (data.error) {
        ...
        $('#old-key-name').val(data.userparam.key);
        ...
        for(var i in users){
          ...
          if(users[i].userId != null){
            updateDepartmentGroupView.handleAddExecutor(users[i].userId);
          }
        }
      }
    };
  ...
  loadWebankUserData: function (targetId) {
    var model = this.model;
    $(targetId).select2({
      placeholder: wtssI18n.system.userPro,//默认文字提示
      multiple: false,
